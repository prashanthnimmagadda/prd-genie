...
export class SourceService {
  ...
  async add(projectId: string, name: string, buffer: Buffer) {
    ...
    const source = {
      ...
      hash,
      binaryPath,
      status: parsed.partial ? 'partial' : 'ready',
    ...
    void this.indexEmbeddings(allChunks.map((chunk) => ({ id: chunk.id, content: chunk.content })));
    return {
      id: source.id,
  ...
  private async indexEmbeddings(items: Array<{ id: string; content: string }>): Promise<void> {
    const batchSize = 24;
    for (let offset = 0; offset < items.length; offset += batchSize) {
      const batch = items.slice(offset, offset + batchSize);
      let vectors: number[][];
      try {
        vectors = await this.embeddings.embed(batch.map((item) => item.content));
      } catch {
        return;
      }
      this.database.sqlite.transaction(() => {
        ...
        batch.forEach((item, index) => {
          ...
          if (!vector || vector.length !== embeddingModel.dimensions) return;
